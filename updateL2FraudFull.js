...
// 2. We need to modify Consumer UI to handle fraudResult.status === "REJECTED" and "APPROVED" explicitly.
// Find the "Step 3 — Result or Submit" part.
const oldResultUI = `{fraudResult.shouldRetake ? (
                  <>
...
const newResultUI = `{fraudResult.shouldRetake ? (
                  <>
                    <AlertTriangle style={{ width: "36px", height: "36px", color: "#FF9900" }} />
                    <h3 style={{ fontSize: "15px", fontWeight: 600, margin: 0, color: "#0F1111" }}>Photo Retake Required</h3>
                    <p style={{ fontSize: "13px", color: "#565959", margin: 0, lineHeight: 1.5 }}>{fraudResult.retakeReason || "Please provide a clearer photo of the item."}</p>
...
if (content.includes('{fraudResult.shouldRetake ? (')) {
  content = content.replace(oldResultUI, newResultUI);
}

// 3. We need to replace the ENTIRE Admin UI block.
// The Admin UI starts at: `// ─── Admin UI ───────────────────────────────────────────────────`
if (!content.includes('const [selectedClaim')) {
  // We don't have selectedClaim yet. We need to add it right inside the component at the top, e.g. next to fraudResult.
  content = content.replace('  const [fraudResult, setFraudResult] = React.useState<any>(null);', '  const [fraudResult, setFraudResult] = React.useState<any>(null);\n  const [selectedClaim, setSelectedClaim] = React.useState<any>(null);');
}
                 ...
                 <div className="flex flex-col gap-4">
                   ...
                   <div className="mt-auto flex flex-col gap-2.5 pt-4 border-t border-slate-100">
                     <button className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold rounded-xl shadow-sm transition-colors" onClick={() => { 
                         setManualReviewQueue((prev: any[]) => prev.filter(c => c.id !== selectedClaim.id)); 
...
console.log("Successfully rebuilt Full Admin UI and Consumer Sync in L2Fraud.tsx!");
