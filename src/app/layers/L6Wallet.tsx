...
function ReturnReasonModal({ order, onClose, onRouteDecision }: ReturnReasonModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Default api fallback for defective is damaged_product
...
export default function L6Wallet() {
  ...
  const routeToL5DarkStore = (order: any) => {
    ...
    setReasonOrder(null);
    setRefundResult({
      message: "Return Approved! Please drop off your item at the nearest Dark Store for inspection.",
  ...
  const handleReturnClick = (order: any) => {
    if (hasManual(order.sku)) {
      ...
      setReasonOrder(order); // Opens reason modal directly
    }
  };
        ...
        {orders.length === 0 ? (
          ...
          <div className="flex flex-col gap-2.5">
            {orders.map((order: any) => {
              ...
              const catalogEntry = PRODUCT_CATALOG.find(p => p.sku === order.sku);
              const isNonReturnable = catalogEntry?.nonReturnable === true;
              const isReturned = (inspectQueue || []).some((q: any) => q.orderId === order.orderId) || (ledgerRecords || []).some((r: any) => r.orderId === order.orderId);
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0 flex-wrap sm:flex-nowrap">
                    ...
                    {/* Return button — gated on policy + return window */}
                    {isReturned ? (
                      <span className="flex items-center gap-1 text-[11px] font-bold text-indigo-600 bg-indigo-50 border border-indigo-200 px-3 py-1.5 rounded-xl">
                        <CheckCircle className="w-3 h-3" />
                        Return Processed
                      </span>
                    ) : isNonReturnable ? (
                      <span className="flex items-center gap-1 text-[10px] font-bold text-slate-400 bg-slate-100 border border-slate-200 px-2.5 py-1.5 rounded-xl" title="This product is non-returnable per hygiene policy">
